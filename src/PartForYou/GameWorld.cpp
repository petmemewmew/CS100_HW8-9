...
//    std::list<GameObject*>::iterator it;
    ...
    std::string bar = "HP: " + std::to_string(player->get_life()) + "/100   Meteors: " +
                      std::to_string(player->meteor_count_fun()) + "   Lives: " + std::to_string(Game_life) +
                      "   Level: " +
...
bool GameWorld::is_crash(GameObject *object1, GameObject *object2) {
    double d = pow(pow(object1->GetX() - object2->GetX(), 2) + pow(object1->GetY() - object2->GetY(), 2), 0.5);
...
GameObject *GameWorld::iterate_crash(GameObject *object) {
    for (auto it = object_list.begin(); it != object_list.end(); it++) {
        if ((*it)->return_type() == "Enemy" || (*it)->return_type() == "Red_Bullet" ||
            (*it)->return_type() == "Blue_Bullet" || (*it)->return_type() == "Meteor" ||
            (*it)->return_type() == "Goodie") {
